...
if (isset($_GET['product_id'])) {
    ...
    $sql = "SELECT * FROM products INNER JOIN categories on products.category_id = categories.category_id WHERE product_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['size']) && isset($_POST['quantity'])) {
        ...
        $insertVariantSql = "INSERT INTO product_variants (product_id, size) VALUES (?,?)";
        ...
        $insertCartItemSql = "INSERT INTO cart_items (user_id, product_variant_id, quantity, price) VALUES (?,?,?,?)";
        $stmt = $conn->prepare($insertCartItemSql);
        $stmt->bind_param('iiid', $userId, $variantId, $quantity, $price);
        $stmt->execute();
        $stmt->free_result();
...
<div class="top-nav">
    ...
    <div class="product-details-main">
        <div class="prod-main-img" style="background-image: url('Admin/<?php echo $product['prod_image']; ?>')"></div>
        <div class="product-details">
            <div class="product-details-head">
                <h1><?php echo $product['prod_name'];?></h1>
                <h2><?php echo $product['gender'];?></h2>
            </div>
            <h1 id="product-price">R<?php echo $product['price'];?></h1>
            <p>Size</p>
            <div class="sizes">
            ...
            <div class="product-details-btn">
                <div class="cart-quantity">
                    <button type="button" onclick="decrease()"><div class="minus"><i class='bx bx-minus'></i></div></button>
                    <p id="value">0</p>
                    <button type="button" onclick="increase()"><div class="plus"><i class='bx bx-plus'></i></div></button>
                    
                </div>
                <form id="add-to-cart" method="POST" action="product-details.php?product_id=<?php echo $productId; ?>">
                    <input type="hidden" name="size" id="selected-size" value="">
                    <input type="hidden" name="quantity" id="selected-quantity" value="0">
